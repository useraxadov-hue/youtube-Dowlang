...
def download_video_sync(url):
    """Videoni yt-dlp yordamida yuklab olish (Sinxron funksiya)"""
    ydl_opts = {
        # Telegram bot standart API cheklovi 50MB. Shuning uchun mos sifatni tanlaymiz.
...
async def handle_links(message: Message):
    try:
        # Matnni JSON formatida ro'yxatga aylantirish
    ...
    for url in links:
        try:
            # Yuklash jarayoni botni qotirib qo'ymasligi uchun alohida oqimda (thread) bajaramiz
            ...
            # Xotirani to'ldirmaslik uchun yuborib bo'lingach faylni o'chiramiz
            if os.path.exists(filename):
                os.remove(filename)
                
        except Exception as e:
            await message.answer(f"❌ Quyidagi havolani yuklashda xatolik yuz berdi: {url}\nSebab: `{str(e)}`", parse_mode="Markdown")
